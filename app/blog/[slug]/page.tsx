...
export default async function BlogPostPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const currentVersion = await getCurrentArticleVersion(slug)
        ...
        <header className="mb-12 space-y-6">
          ...
          {currentVersion && (
            <div className="flex items-center gap-3 text-sm">
              <VersionBadge versionLabel={currentVersion.versionLabel} />
              {currentVersion.publishedDate && (
                <div className="flex items-center gap-1.5 text-muted-foreground">
                  <Calendar className="h-3.5 w-3.5" />
                  <span>
                    Mis à jour le {new Date(currentVersion.publishedDate).toLocaleDateString('fr-FR', {
                      day: 'numeric',
                      month: 'long',
                      year: 'numeric'
                    })}
          ...
          {hasVersions && (
            <Link
              ...
              className="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium bg-muted hover:bg-muted/80 border border-border rounded-lg transition-colors"
            >
              <History className="h-4 w-4" />
              Voir l'historique ({versions.length} version{versions.length > 1 ? 's' : ''})
            </Link>
          )}
